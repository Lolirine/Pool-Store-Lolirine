...
import { ShippingPolicyPageProps } from '../types';
import GoBackButton from '../components/GoBackButton';

const ShippingPolicyPage: React.FC<ShippingPolicyPageProps> = ({ pageContent, goBack, canGoBack, navigateTo }) => {
    if (!pageContent) {
        return (
    ...
    const { hero, sections, cta } = pageContent;
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl py-16">
                ...
                {cta && (
                    <div className="mt-8 bg-gray-100 p-6 rounded-lg text-center">
                        <p className="text-gray-700">{cta.text}</p>
                        <button onClick={() => navigateTo(cta.page)} className="mt-4 px-6 py-2 bg-cyan-600 text-white font-semibold rounded-lg hover:bg-cyan-700 transition-colors">
                            {cta.buttonText}
                        </button>
                    </div>
                )}
